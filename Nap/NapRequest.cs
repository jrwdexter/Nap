		...
		private void RunBooleanPluginMethod(Func<IPlugin, bool> pluginMethod, string errorMessage)
		{
			try
			{
				if (!NapSetup.Plugins.Aggregate(true, (current, plugin) => current && pluginMethod.Invoke(plugin)))
					throw new Exception(errorMessage); // TODO - make specific exception
			}
			catch (Exception e)
			{
				throw new Exception($"{errorMessage} See inner exception for details.", e);	// TODO: Make specific exception
			}
		}
